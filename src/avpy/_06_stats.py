...
def plot_nerve(nerve_map, 
    ...
    for segment_name, start_slice, end_slice in segments:
        ...
        ax.text(110, slice_position, segment_name, color='white', fontsize=18, zorder=100)
    ...
    for feature in features:
        for side in sides:
            ...
            for y in range(n_slices):
                ...
                df_slice_a = filter_dataframe(df, current_slice_yz=[y], group=[dataset_a])
                df_slice_b = filter_dataframe(df, current_slice_yz=[y], group=[dataset_b])
